    ...
    """
        ...
        for iteration in range(self.max_iterations):
            ...
            for i in range(len(trainingData)):
                ...
                highestscore = None
                myY = None
                datum = trainingData[randomrange[i]].copy()
                ...
                for y in self.legalLabels:
                    ...
                    if score > highestscore or highestscore is None:
                        highestscore = score
                        myY = y
                    perceptronScore = datum * self.weights[y] + bias[y]
                ...
                if myY != actualY:
                    #print "no match"
                    mira = abs(((self.weights[myY] - self.weights[actualY]) * datum + 1.0) / (2.0 * (datum * datum)))
                    c = 0.01

                    tau = min(c,mira)

                    datumWithTau = trainingData[randomrange[i]].copy()
                    for key in datum:
